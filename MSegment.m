function [output,tarea]=MSegment(inp)
...
I = imfill(I,'holes');
[out tc] = bwlabel(I,8);
tprop = regionprops(out,'Area');

for i=1:1:tc
   tar(i) = tprop(i).Area;
end
marea = max(tar); mth = marea - min(tar);

output = bwareaopen(out,mth);
...
disp('No of Defect areas: ');
disp(Pcount);

%%%%%%% Tumor area by counting no of pixels
