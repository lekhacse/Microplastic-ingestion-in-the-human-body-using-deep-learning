...
cd ..       
   ...
   axes(handles.axes1);
   imshow(im);
   title('Input Microscopic tissue cells Image');
...
if isequal(cout,1)
   ...
   set(handles.text1,'String','No Microplastics Found');
   
elseif isequal(cout,2)
    
  set(handles.text6,'String','Stage :'); 
  set(handles.text1,'String','Above 50% Plastics Found');
   
elseif isequal(cout,3)
    
  set(handles.text6,'String','Stage :'); 
  set(handles.text1,'String','Below 20% Plastics Found');
elseif isequal(cout,4)
    
  set(handles.text6,'String','Stage :'); 
  set(handles.text1,'String','Above 70% Plastics');
...
%%%%Parameters Evaluation %%%%%%total number of test samples 9
   Tp = 3; Fn = 2;  %%%%%%%after classification
...
elseif result ==2
   ...
   axes(handles.axes1); 
   imshow(inp); title('LEPD - Above 50% Plastics');
   hold on;
...
elseif result ==3 
   ...
   boundary = bwboundaries(im2bw(segout));
   axes(handles.axes1);
   imshow(inp); title('PE - Below 20% Plastics');
   hold on;
   for ii=1:1:length(boundary)
       btemp = boundary{ii};
       plot(btemp(:,2),btemp(:,1),'r','LineWidth',4);
...
elseif result ==4 
   ...
   boundary = bwboundaries(im2bw(segout));
   axes(handles.axes1);
   imshow(inp); title('PHA - Above 70% Plastics');
   hold on;
   for ii=1:1:length(boundary)
       btemp = boundary{ii};
       plot(btemp(:,2),btemp(:,1),'r','LineWidth',4);
 ...
 elseif result ==5 
   ...
   boundary = bwboundaries(im2bw(segout));
   axes(handles.axes1);
   imshow(inp); title('PHA - Below 20% Plastics');
   hold on;
   for ii=1:1:length(boundary)
       btemp = boundary{ii};
       plot(btemp(:,2),btemp(:,1),'r','LineWidth',4);
